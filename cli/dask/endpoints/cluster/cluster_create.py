...
def setup_parser(parser: argparse.ArgumentParser):
    ...
    parser.add_argument('--password',
                        help="The password to access your dask cluster's head \
...
def execute(args: typing.NamedTuple):
    ...
    # create dask cluster
    cluster = dask_client.create_cluster(
        aztk.dask.models.ClusterConfiguration(
            ...
            dask_configuration=aztk.dask.models.DaskConfiguration() #TODO: implement
        ),
        wait=cluster_conf.wait
    )

    if cluster_conf.username:
        ssh_key = dask_client.secrets_config.ssh_pub_key

        ssh_key, password = utils.get_ssh_key_or_prompt(
            ssh_key, cluster_conf.username, cluster_conf.password, dask_client.secrets_config)

        dask_client.create_user(
            cluster_id=cluster_conf.uid,
...
def print_cluster_conf(cluster_conf):
    log.info("-------------------------------------------")
    log.info("dask cluster id:        %s", cluster_conf.uid)
    log.info("dask cluster size:      %s",
    ...
    log.info("dask cluster vm size:   %s", cluster_conf.vm_size)
