...
def generate_user_on_pool(base_operations, pool_id, nodes):
    ...
    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = {
            executor.submit(base_operations.create_user_on_node, pool_id, node.id, generated_username, ssh_pub_key): node
            for node in nodes
        }
