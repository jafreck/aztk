...
def __app_submit_cmd(
    ...
    with open (os.path.join(hadoop_home, 'conf', 'master')) as f:
    ...
    HADOOP_SUBMit_cmd = CommandBuilder(
        '{0}/bin/hadoop-submit'.format(hadoop_home))
    HADOOP_SUBMit_cmd.add_option(
        '--master', 'hadoop://{0}:7077'.format(master_ip))
    HADOOP_SUBMit_cmd.add_option('--name', name)
    HADOOP_SUBMit_cmd.add_option('--class', main_class)
    HADOOP_SUBMit_cmd.add_option('--jars', jars and ','.join(jars))
    HADOOP_SUBMit_cmd.add_option('--py-files', py_files and ','.join(py_files))
    HADOOP_SUBMit_cmd.add_option('--files', files and ','.join(files))
    HADOOP_SUBMit_cmd.add_option('--driver-java-options', driver_java_options)
    HADOOP_SUBMit_cmd.add_option('--driver-library-path', driver_library_path)
    HADOOP_SUBMit_cmd.add_option('--driver-class-path', driver_class_path)
    HADOOP_SUBMit_cmd.add_option('--driver-memory', driver_memory)
    HADOOP_SUBMit_cmd.add_option('--executor-memory', executor_memory)
    if driver_cores:
        HADOOP_SUBMit_cmd.add_option('--driver-cores', str(driver_cores))
    if executor_cores:
        HADOOP_SUBMit_cmd.add_option('--executor-cores', str(executor_cores))

    HADOOP_SUBMit_cmd.add_argument(
        os.path.expandvars(app) + ' ' +
        ' '.join(['\'' + str(app_arg) + '\'' for app_arg in (app_args or [])]))

    with open("hadoop-submit.txt", mode="w", encoding="UTF-8") as stream:
        stream.write(HADOOP_SUBMit_cmd.to_str())

    return HADOOP_SUBMit_cmd
...
def upload_log(blob_client, application):
    ...
    '''
    log_file = os.path.join(os.environ['AZ_BATCH_TASK_WORKING_DIR'], os.environ['HADOOP_SUBMIT_LOGS_FILE'])
...
if __name__ == "__main__":
    ...
    # force batch task exit code to match hadoop exit code
