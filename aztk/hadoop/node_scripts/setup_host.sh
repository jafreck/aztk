...
run_docker_container () {
    ...
    if [ "$(docker ps -a -q -f name=$container_name)" ]; then
        ...
        echo "Waiting for hadoop docker container to setup."
        # docker exec hadoop /bin/bash -c '$AZTK_WORKING_DIR/.aztk-env/.venv/bin/python $AZTK_WORKING_DIR/aztk/hadoop/node_scripts/wait_until_setup_complete.py'
...
main () {
    ...
    chmod 777 $AZTK_WORKING_DIR/aztk/hadoop/node_scripts/docker_main.sh
    ...
    cp $AZTK_WORKING_DIR/aztk/hadoop/node_scripts/Pipfile $AZTK_WORKING_DIR/.aztk-env
    cp $AZTK_WORKING_DIR/aztk/hadoop/node_scripts/Pipfile.lock $AZTK_WORKING_DIR/.aztk-env
