...
from install import pick_master, hadoop, scripts, create_user, plugins, hadoop_container
...
def setup_host(docker_repo: str):
    ...
    hadoop_container.start_hadoop_container(
...
def setup_hadoop_container():
    """
    Code run in the main hadoop container
    """
    is_master = os.environ.get("AZTK_IS_MASTER") == "true"
    is_worker = os.environ.get("AZTK_IS_WORKER") == "true"
    print("Setting hadoop container. Master: ", is_master, ", Worker: ", is_worker)

    print("Copying hadoop setup config")
    hadoop.setup_conf()
    print("Done copying hadoop setup config")

    hadoop.setup_connection()

    if is_master:
        hadoop.start_hadoop_master()

    if is_worker:
        hadoop.start_hadoop_worker()

    plugins.setup_plugins(target=PluginTarget.hadoopContainer, is_master=is_master, is_worker=is_worker)
