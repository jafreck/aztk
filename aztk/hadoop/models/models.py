...
class hadoopToolkit(aztk.models.Toolkit):
...
class Cluster(aztk.models.Cluster):
    ...
    def is_pool_running_hadoop(self, pool: batch_models.CloudPool):
        if pool.metadata is None:
            return False

        for metadata in pool.metadata:
            if metadata.name == constants.AZTK_SOFTWARE_METADATA_KEY:
                return metadata.value == aztk.models.Software.hadoop
...
class hadoopConfiguration(Model):
    hadoop_defaults_conf = fields.String(default=None)
    hadoop_env_sh = fields.String(default=None)
...
class ClusterConfiguration(aztk.models.ClusterConfiguration):
    hadoop_configuration = fields.Model(hadoopConfiguration, default=None)
...
class JobConfiguration:
    ...
    def to_cluster_config(self):
        return ClusterConfiguration(
            ...
            hadoop_configuration=self.hadoop_configuration,
