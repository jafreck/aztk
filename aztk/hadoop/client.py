...
from aztk.hadoop.helpers import create_cluster as create_cluster_helper
from aztk.hadoop.helpers import get_log as get_log_helper
from aztk.hadoop.helpers import job_submission as job_submit_helper
from aztk.hadoop.helpers import submit as cluster_submit_helper
from aztk.hadoop.helpers import cluster_diagnostic_helper
from aztk.hadoop.utils import util
...
class Client(BaseClient):
    """
    Aztk hadoop Client
    This is the main entry point for using aztk for hadoop

    Args:
        secrets_config(aztk.hadoop.models.models.SecretsConfiguration): Configuration with all the needed credentials
    """

    def create_cluster(self, cluster_conf: models.ClusterConfiguration, wait: bool = False):
        """
        Create a new aztk hadoop cluster

        Args:
            cluster_conf(aztk.hadoop.models.models.ClusterConfiguration): Configuration for the the cluster to be created
            wait(bool): If you should wait for the cluster to be ready before returning

        Returns:
            aztk.hadoop.models.Cluster
        """
    ...
    def list_clusters(self):
        try:
            return [models.Cluster(pool) for pool in self.__list_clusters(aztk.models.Software.hadoop)]
        except batch_error.BatchErrorException as e:
            raise error.AztkError(helpers.format_batch_exception(e))
    ...
    def cluster_run(self, cluster_id: str, command: str, host=False, internal: bool = False, timeout=None):
        try:
            return self.__cluster_run(cluster_id,
                                      command,
                                      internal,
                                      container_name='hadoop' if not host else None,
                                      timeout=timeout)
        except batch_error.BatchErrorException as e:
            raise error.AztkError(helpers.format_batch_exception(e))
    ...
    def cluster_copy(self, cluster_id: str, source_path: str, destination_path: str, host: bool = False, internal: bool = False, timeout: int = None):
        try:
            container_name = None if host else 'hadoop'
