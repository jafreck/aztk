...
def __get_node(hadoop_client, node_id: str, cluster_id: str) -> batch_models.ComputeNode:
    return hadoop_client.batch_client.compute_node.get(cluster_id, node_id)


def generate_task(hadoop_client, container_id, application, remote=False):
    resource_files = []
    ...
    task_cmd.add_argument('hadoop /bin/bash >> output.log 2>&1')
    task_cmd.add_argument('-c "source ~/.bashrc; ' \
                          'export PYTHONPATH=$PYTHONPATH:\$AZTK_WORKING_DIR; ' \
                          'cd \$AZ_BATCH_TASK_WORKING_DIR; ' \
                          '\$AZTK_WORKING_DIR/.aztk-env/.venv/bin/python \$AZTK_WORKING_DIR/aztk/hadoop/node_scripts/submit.py"')
...
def affinitize_task_to_master(hadoop_client, cluster_id, task):
    cluster = hadoop_client.get_cluster(cluster_id)
    ...
    master_node = hadoop_client.batch_client.compute_node.get(pool_id=cluster_id, node_id=cluster.master_node_id)
    task.affinity_info = batch_models.AffinityInformation(affinity_id=master_node.affinity_id)
    return task


def submit_application(hadoop_client, cluster_id, application, remote: bool = False, wait: bool = False):
    """
    Submit a hadoop app
    """
    task = generate_task(hadoop_client, cluster_id, application, remote)
    task = affinitize_task_to_master(hadoop_client, cluster_id, task)


    # Add task to batch job (which has the same name as cluster_id)
    job_id = cluster_id
    hadoop_client.batch_client.task.add(job_id=job_id, task=task)

    if wait:
        helpers.wait_for_task_to_complete(job_id=job_id, task_id=task.id, batch_client=hadoop_client.batch_client)
