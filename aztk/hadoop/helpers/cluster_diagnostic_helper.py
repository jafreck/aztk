...
def run(hadoop_client, cluster_id, output_directory=None):
    # copy debug program to each node
    output = hadoop_client.cluster_copy(cluster_id, os.path.abspath("./aztk/hadoop/utils/debug.py"), "/tmp/debug.py", host=True)
    ssh_cmd = _build_diagnostic_ssh_command()
    run_output = hadoop_client.cluster_run(cluster_id, ssh_cmd, host=True)
    remote_path = "/tmp/debug.zip"
    if output_directory:
        local_path = os.path.join(os.path.abspath(output_directory), "debug.zip")
        output = hadoop_client.cluster_download(cluster_id, remote_path, local_path, host=True)
        ...
        output = hadoop_client.cluster_download(cluster_id, remote_path, host=True)
