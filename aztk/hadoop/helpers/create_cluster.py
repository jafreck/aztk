...
from aztk.hadoop.models import ClusterConfiguration
...
def __get_docker_credentials(hadoop_client):
    creds = []
    docker = hadoop_client.secrets_config.docker
    if docker:
        if docker.endpoint:
...
def __get_secrets_env(hadoop_client):
    shared_key = hadoop_client.secrets_config.shared_key
    service_principal = hadoop_client.secrets_config.service_principal
    if shared_key:
        return [
...
def __cluster_install_cmd(zip_resource_file: batch_models.ResourceFile,
    ...
    """
        ...
        to be run on the start task of the pool to setup hadoop.
    """
    ...
    setup = [
        'time('\
            ...
            'chmod 777 $AZ_BATCH_TASK_WORKING_DIR/aztk/hadoop/node_scripts/setup_host.sh;'\
        ') 2>&1'.format(zip_resource_file.file_path),
        '/bin/bash $AZ_BATCH_TASK_WORKING_DIR/aztk/hadoop/node_scripts/setup_host.sh {0} {1}'.format(
            constants.DOCKER_HADOOP_CONTAINER_NAME,
            docker_repo,
        )
    ]
...
def generate_cluster_start_task(
    ...
    hadoop_web_ui_port = constants.DOCKER_HADOOP_WEB_UI_PORT
    # hadoop_worker_ui_port = constants.DOCKER_hadoop_WORKER_UI_PORT
    # hadoop_job_ui_port = constants.DOCKER_hadoop_JOB_UI_PORT

    hadoop_container_name = constants.DOCKER_HADOOP_CONTAINER_NAME
    hadoop_submit_logs_file = constants.HADOOP_SUBMIT_LOGS_FILE

    # TODO use certificate
    environment_settings = __get_secrets_env(hadoop_client) + [
        batch_models.EnvironmentSetting(
            name="HADOOP_WEB_UI_PORT", value=hadoop_web_ui_port),
        batch_models.EnvironmentSetting(
            name="AZTK_GPU_ENABLED", value=helpers.bool_env(gpu_enabled)),
        batch_models.EnvironmentSetting(
            name="HADOOP_SUBMIT_LOGS_FILE", value=hadoop_submit_logs_file),
    ] + __get_docker_credentials(hadoop_client) + _get_aztk_environment(cluster_id, worker_on_master, mixed_mode)
