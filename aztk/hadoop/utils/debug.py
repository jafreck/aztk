...
def get_docker_containers(docker_client):
    ...
    try:
        ...
        for container in containers:
            ...
            if container.name == "hadoop":    #TODO: find a more robust way to get specific info off specific containers
                logs.extend(get_container_aztk_script(container))
                logs.extend(get_hadoop_logs(container))
                logs.extend(get_hadoop_app_logs(container))
...
def get_hadoop_logs(container):
    hadoop_logs_path = "/home/hadoop-current/logs"
    try:
        stream, _ = container.get_archive(hadoop_logs_path)    # second item is stat info
        return extract_tar_in_memory(container, stream)
    except docker.errors.APIError as e:
        return [(container.name + "/" + "hadoop-logs.err", e.__str__())]


def get_hadoop_app_logs(container):
    hadoop_app_logs_path = "/home/hadoop-current/work"
    try:
        stream, _ = container.get_archive(hadoop_app_logs_path)
        return extract_tar_in_memory(container, stream)
    except docker.errors.APIError as e:
        return [(container.name + "/" + "hadoop-work-logs.err", e.__str__())]
