...
def _list_applications(spark_job_operations, job_id):
    ...
    for task in spark_job_operations.batch_client.task.list(recent_run_job.id):
        if task.id != job_id:
...
def list_applications(spark_job_operations, job_id):
    try:
        applications = _list_applications(spark_job_operations, job_id)
