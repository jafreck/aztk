...
def setup_parser(parser: argparse.ArgumentParser):
    parser.add_argument('--id', dest="cluster_id", help='The unique id of your gatk cluster')
    parser.add_argument('--webui', help='Local port to port gatk\'s master UI to')
    parser.add_argument('--jobui', help='Local port to port gatk\'s job UI to')
    parser.add_argument('--jobhistoryui', help='Local port to port gatk\'s job history UI to')
    parser.add_argument('-u', '--username', help='Username to gatk cluster')
    parser.add_argument('--host', dest="host", action='store_true', help='Connect to the host of the gatk container')
...
def execute(args: typing.NamedTuple):
    ...
    cluster_config = gatk_client.get_cluster_config(args.cluster_id)
    ssh_conf = SshConfig()
    ...
    log.info("-------------------------------------------")
    utils.log_property("gatk cluster id", ssh_conf.cluster_id)
    ...
    try:
        ...
        if not ssh_conf.connect:
            log.info("")
            log.info("Use the following command to connect to your gatk head node:")
            log.info("\t%s", ssh_cmd)
